...
var gitignoreEntries = []string{
	".env",
	// The whole .claude/ is local: settings.json holds per-user config (telemetry
	// endpoints, env) that must not be committed, and the skills/commands/agents are
	// regenerated by `themis skills install`. Nothing here belongs in the repo.
	".claude/",
	".themis/state.json",
