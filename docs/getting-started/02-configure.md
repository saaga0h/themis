...
# provider: github            # host: github | gitea — usually auto-detected from your git remote
...
- **`provider`** — the host this repo lives on, where the factory reads issues and opens PRs. **Usually you leave it unset** — it's auto-detected from your git `origin` remote (`github.com` → `github`, any other host → `gitea`). Set `provider: github|gitea` only to force it; the `--provider` flag overrides per-run; with no remote it defaults to `github`. See [`docs/providers.md`](../providers.md) for the per-provider tooling.
