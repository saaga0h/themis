...
func runInit(args []string) error {
	...
	fmt.Println("     Put a linux themis binary in this dir as ./themis (the COPY line needs it):")
	fmt.Println("       cp /path/to/themis/bin/themis ./themis     # a linux binary (e.g. make build)")
	fmt.Println("     Then build — -t is a lowercase tag YOU choose (not the filename), set the")
	fmt.Println("     same tag as image: in workflow.yaml:")
