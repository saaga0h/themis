...
func runInit(args []string) error {
	...
	fmt.Println("  2. Containerfile — fill the toolchain TODO for your stack. themis itself is built")
	fmt.Println("     from source inside the image for the right arch (no binary to supply); override")
	fmt.Println("     THEMIS_REPO to build from your own git host. Then build — -t is a lowercase tag")
	fmt.Println("     YOU choose (not the filename), set the same tag as image: in workflow.yaml:")
