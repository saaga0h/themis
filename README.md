...
| `/issue <number> [--provider github\|gitea]` | Implement a single issue end-to-end, fully autonomously |
| `/factory [--provider github\|gitea] [--max N] [--dry-run]` | Process all open `ready-for-agent` issues in order |
...
a sandboxed container. This gives you a software factory: issues as the work
queue, Claude Code as the implementer, a container as the blast radius boundary.
Both commands support `--provider github|gitea` to work with either GitHub or a
self-managed Gitea instance.
...
- `GH_TOKEN` — a GitHub fine-grained token with Issues (read/write) and Pull requests (read/write) **[GitHub only]**
- `GITEA_TOKEN` — a Gitea personal access token with Issues and Pull requests permissions **[Gitea only]**
...
GH_TOKEN=your_github_token_here        # GitHub only
GITEA_TOKEN=your_gitea_token_here      # Gitea only
```
...
Then: `make run-factory`, `make run-issue ISSUE=42`, `make dry-run`. Override the provider per-run with `make run-factory PROVIDER=gitea`.

**Gitea**

For projects on a self-managed Gitea instance, set `PROVIDER ?= gitea` as the default:
...
- Store tokens in `.env` and add `.env` to `.gitignore`
...
- [x] `/issue` command — fully autonomous issue implementation
